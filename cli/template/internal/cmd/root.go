...
import (
	iconfig "go.zenithar.org/repo-template/cli/template/internal/config"
...
var mainCmd = &cobra.Command{
	Use:   "template",
	Short: "Golang Microservice Template",
}

func init() {
	mainCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (config.toml)")
	mainCmd.AddCommand(versionCmd)
	mainCmd.AddCommand(cmdcfg.NewConfigCommand(conf, "TMPL"))
	// ADD Commands here
}
...
func initConfig() {
	if err := config.Load(conf, "TMPL", cfgFile); err != nil {
